...
/*   Updated: 2025/07/17 21:56:53 by jde-carv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_entity	*new_background(t_game *game, char *image_path)
{
	t_entity	*background;
	...
	background->sprite->img = mlx_xpm_file_to_image(game->mlx, image_path, &background->sprite->w, &background->sprite->h);
...
void e_background(t_game *game)
{
	...
	//change library and default strdup.
	image_path = strdup("assets/background/back-1.xpm");
	index = 0;
	while (index < 5)
	{
		image_path[23] = index + '0' + 1;
		list[index] = new_background(game, image_path);
