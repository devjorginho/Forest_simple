/* ************************************************************************** */
/* */
/* :::      ::::::::   */
/* systems.c                                          :+:      :+:    :+:   */
/* +:+ +:+         +:+     */
/* By: devjorginho <devjorginho@student.42.fr>    +#+  +:+       +#+        */
/* +#+#+#+#+#+   +#+           */
/* Created: 2025/06/21 16:35:42 by devjorginho       #+#    #+#             */
/* Updated: 2025/10/20 13:10:40 by devjorginho      ###   ########.fr       */
/* */
/* ************************************************************************** */

#include "../inc/so_long.h"

void    draw_system(t_game *game)
{
    t_dataimg dataimg;
    int i;

    i = 0;
    while(i < game->count_entities)
    {
        if ((game->entities[i].sprite) && game->entities[i].position)
        {
            dataimg.image = game->entities[i].sprite->img;
            dataimg.width = game->entities[i].sprite->w;
            dataimg.height = game->entities[i].sprite->h;
            if(i == 4 || i == 8 || i == 12)
                game->entities[i].position->x = game->entities[i].position->x - game->parallax * 0.90;
            if(i == 3 || i == 7 || i == 11)
                game->entities[i].position->x = game->entities[i].position->x - game->parallax * 0.45;
            if(i == 2 || i == 6 || i == 10)
                game->entities[i].position->x = game->entities[i].position->x - game->parallax * 0.27;
            if(i == 1 || i == 5 || i == 9)
                game->entities[i].position->x = game->entities[i].position->x - game->parallax * 0.09;
            draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
        }
        else if(game->entities[i].image && game->entities[i].position)
        {
            if (game->entities[i].animation->is_jumping && game->entities[i].animation->last_direction == 0 && game->entities[i].velocity->y < 0)
            {
                dataimg.image = game->entities[i].image->jumpr_img[0];
                dataimg.width = 34;
                dataimg.height = 68;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else if (game->entities[i].animation->is_jumping && game->entities[i].animation->last_direction == 1 && game->entities[i].velocity->y < 0)
            {
                dataimg.image = game->entities[i].image->jumpl_img[0];
                dataimg.width = 34;
                dataimg.height = 68;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else if (game->entities[i].velocity->y > 0 && game->entities[i].animation->last_direction == 1)
            {
                dataimg.image = game->entities[i].image->landingl_img[0];
                dataimg.width = 40;
                dataimg.height = 70;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else if (game->entities[i].velocity->y > 0 && game->entities[i].animation->last_direction == 0)
            {
                dataimg.image = game->entities[i].image->landingr_img[0];
                dataimg.width = 40;
                dataimg.height = 70;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else if (game->entities[i].animation->p_runing_r)
            {
                dataimg.image = game->entities[i].image->rr_img[(game->n_frames / 8) % 8];
                dataimg.width = 42;
                dataimg.height = 66;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else if (game->entities[i].animation->p_runing_l)
            {
                dataimg.image = game->entities[i].image->rl_img[(game->n_frames / 8) % 8];
                dataimg.width = 42;
                dataimg.height = 66;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else if (game->entities[i].animation->last_direction == 0)
            {
                dataimg.image = game->entities[i].image->idler_img[(game->n_frames / 8) % 12];
                dataimg.width = 38;
                dataimg.height = 68;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
            else
            {
                dataimg.image = game->entities[i].image->idlel_img[(game->n_frames / 8) % 12];
                dataimg.width = 38;
                dataimg.height = 68;
                draw_img_to_framebuffer(game, &dataimg, *game->entities[i].position);
            }
        }
        i++;
    }
}

void    float_item_system(t_game *game)
{

	int i;
	
	i = 0;
	
	while(i < game->count_entities)
	{
		if (game->entities[i].position && game->entities[i].float_item)
		{
			game->entities[i].position->y = game->entities[i].position->y
				+ game->entities[i].float_item->dir * 0.5;
			if (game->entities[i].position->y > game->entities[i].float_item->initial_y + 10)
				game->entities[i].float_item->dir = -1;
			if (game->entities[i].position->y < game->entities[i].float_item->initial_y - 10)
				game->entities[i].float_item->dir = 1;
		}
		i++;
	}
}

void    gravity_system(t_game *game)
{
    int i;
    
    const float GLOBAL_SPEED_FACTOR = 12.0f;
    
    const float GRAVITY_ACCEL_ORIGINAL = 0.5f; 
    
    const float JUMP_VELOCITY_ORIGINAL = -10.0f;
    
    const float NORMALIZATION_DIVISOR = 10.0f;

    const float GRAVITY_ACCEL_NORM = (GRAVITY_ACCEL_ORIGINAL * GLOBAL_SPEED_FACTOR) / NORMALIZATION_DIVISOR; 
    const float JUMP_VELOCITY_NORM = (JUMP_VELOCITY_ORIGINAL * GLOBAL_SPEED_FACTOR) / NORMALIZATION_DIVISOR;

    i = 0;
    while(i < game->count_entities)
    {
        if (game->entities[i].gravity && game->entities[i].position && game->entities[i].velocity)
        {
            game->entities[i].velocity->y += GRAVITY_ACCEL_NORM;
            
            if (game->keymap['w'] && !game->entities[i].animation->is_jumping)
            {
                game->entities[i].velocity->y = JUMP_VELOCITY_NORM;
                game->entities[i].animation->is_jumping = 1;
            }
            game->entities[i].position->y += game->entities[i].velocity->y;
        }
        i++;
    }
}

void movement_system(t_game *game)
{
    int i;
    
    const float GLOBAL_SPEED_FACTOR = 12.0f;
    
    const float WALK_SPEED_ORIGINAL = 1.2f; 
    
    const float SPRINT_BONUS_ORIGINAL = 2.4f; 
    
    const float NORMALIZATION_DIVISOR = 10.0f;

    const float BASE_SPEED_NORM = (WALK_SPEED_ORIGINAL * GLOBAL_SPEED_FACTOR) / NORMALIZATION_DIVISOR;
    const float SPRINT_BONUS_NORM = (SPRINT_BONUS_ORIGINAL * GLOBAL_SPEED_FACTOR) / NORMALIZATION_DIVISOR;

    i = 0;
    while(i < game->count_entities)
    {
        if (game->entities[i].velocity && game->entities[i].keyboard && game->entities[i].position)
        {
            float speed = BASE_SPEED_NORM;
            
            if(game->keymap[65505])
                speed += SPRINT_BONUS_NORM;

            if (game->keymap['d'])
            {
                game->entities[i].animation->p_runing_r = 1;
                game->entities[i].animation->p_runing_l = 0;
                game->entities[i].animation->last_direction = 0;
                game->entities[i].velocity->x = speed;
            }
            else if (game->keymap['a'])
            {
                game->entities[i].animation->p_runing_l = 1;
                game->entities[i].animation->p_runing_r = 0;
                game->entities[i].animation->last_direction = 1;
                game->entities[i].velocity->x = -speed;
            }
            else if (game->keymap['h'])
                game->entities[i].animation->is_jumping = 0;
            else
            {
                game->entities[i].animation->p_runing_r = 0;
                game->entities[i].animation->p_runing_l = 0;
                game->entities[i].velocity->x = 0;
            }
            
            game->entities[i].position->x += game->entities[i].velocity->x;
            game->parallax = game->entities[i].velocity->x;
        }       
        i++;
    }
}


void    collision_system(t_game *game)
{
    int i, j;
    t_entity *a;
    t_entity *b;

    for (i = 0; i < game->count_entities; i++)
    {
        a = &game->entities[i];
        
        if (!a->collider || !a->position || !a->velocity || a->collider->solid)
            continue;
        
        int is_player = (a->keyboard != NULL);

        for (j = 0; j < game->count_entities; j++)
        {
            if (i == j)
                continue;

            b = &game->entities[j];
            if (!b->collider || !b->position)
                continue;

            
            if (a->position->x < b->position->x + b->collider->width &&
                a->position->x + a->collider->width > b->position->x &&
                a->position->y < b->position->y + b->collider->height &&
                a->position->y + a->collider->height > b->position->y)
            {
                if (is_player && !b->collider->solid)
                {
                    if (b->float_item) 
                    {
                        b->sprite = NULL;
                        b->position = NULL;
                        
                    }
                    continue; 
                }

                if (b->collider->solid)
                {
                    float mid_a_x = a->position->x + a->collider->width / 2.0f;
                    float mid_b_x = b->position->x + b->collider->width / 2.0f;
                    float mid_a_y = a->position->y + a->collider->height / 2.0f;
                    float mid_b_y = b->position->y + b->collider->height / 2.0f;
                    
                    float half_w_sum = (a->collider->width + b->collider->width) / 2.0f;
                    float half_h_sum = (a->collider->height + b->collider->height) / 2.0f;
                    
                    float dx = mid_a_x - mid_b_x;
                    float dy = mid_a_y - mid_b_y;

                    float overlapX = half_w_sum - fabsf(dx);
                    float overlapY = half_h_sum - fabsf(dy);

                    if (overlapX < overlapY)
                    {
                        if (dx > 0)
                            a->position->x = b->position->x + b->collider->width;
                            a->position->x = b->position->x - a->collider->width;
                        
                        a->velocity->x = 0;
                    }
                    else
                    {
                        if (dy > 0)
                        {
                            a->position->y = b->position->y + b->collider->height;
                            a->velocity->y = 0;
                        }
                        else
                        {
                            a->position->y = b->position->y - a->collider->height;
                            a->velocity->y = 0;
                            a->animation->is_jumping = 0;
                        }
                    }
                }
            }
        }
    }
}